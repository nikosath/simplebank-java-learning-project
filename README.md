...
|--------|---------------------|------------------|-------------------|------------------------|---------|
| [Learning Java Collections](https://www.linkedin.com/learning/learning-java-collections) | [Test Sociably, Fake the Boundaries](test-sociably-fake-the-boundaries.md) *(optional)* | **v1** | Fully working app using in-memory storage. **Suggestion:** use Maps for `O(1)` account lookups, and Streams with Lambda expressions for formatting/filtering transaction history. **Suggestion:** include a solid suite of automated unit tests for your core logic. | Implement **test-sociably, fake-the-boundaries**. | `v1-in-memory` |
...
**Optional stretch goal - Testing Challenge (The Pragmatic Way)**: If you want to push your engineering skills further, read and apply the principles from [test-sociably, fake-the-boundaries](test-sociably-fake-the-boundaries.md). Specifically, try to:
